...
service getBouchon on new http:Listener(9086) {
    ...
    resource function information(http:Caller caller, http:Request req) {
       ...
       string srcFileName = "./societe.txt";
    var result = io:openReadableFile(srcFileName);
    if (result is io:ReadableByteChannel) {
        io:ReadableCharacterChannel characterChannel = new(result, "UTF-8");
        io:println(characterChannel.readJson());
    var result2 = characterChannel.close();
    }
